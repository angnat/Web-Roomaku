...
function fnServerOData(sUrl, aoData, fnCallback, oSettings) {
   // console.log(sUrl);
    ...
    // get default filter
    var defs = {}
    $.each(sUrl.split('?').pop().split('&'), function (i, value) {
        var def = value.split('=');
        defs[def[0]] = def[1];
    });
    if (defs['$filter']) data['$filter'] = defs['$filter'];
    //
    ...
    if (oSettings.oFeatures.bServerSide) {
        ...
        $.each(oSettings.aoColumns,
            function (i, value) {
                ...
                if ((oParams.sSearch !== null && oParams.sSearch !== "" || columnFilter !== null && columnFilter !== "") && value.bSearchable) {
                    switch (value.sType) {
                        case 'datetime_':
                            if (!isNaN(parseFloat(oParams.sSearch) && isFinite(oParams.sSearch))) {
                                var a = oParams.sSearch.split('-').reverse().join('-');
                                var d = new Date(a);
                                if (Object.prototype.toString.call(d) === "[object Date]") {
                                    ...
                                    if (isNaN(d.getDate())) { 
                                        ...
                                        if (oParams.sSearch.length == 10) {
                                            if (oParams.sSearch !== null && oParams.sSearch !== "") {
                                                asFilters.push("(" + sFieldName + " eq DateTime'" + a + "')");
                                            }
        ...
        if (asFilters.length > 0) {
            if (data.$filter !== undefined) {
                data.$filter = " ( " + data.$filter + " ) and ( " + asFilters.join(" or ") + " ) ";
            } else {
        ...
        for (var i = 0; i < oParams.iSortingCols; i++) {
            var a = oParams["mDataProp_" + oParams["iSortCol_" + i]];
            var replace = a.replace(/[.]/g, '/');
            asOrderBy.push(replace + " " + (oParams["sSortDir_" + i] || ""));
        }
